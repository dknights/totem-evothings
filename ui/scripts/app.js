...
var Microbit = function () {
  function Microbit(config) {
    classCallCheck(this, Microbit);

    this.config = config;
  }

  createClass(Microbit, [{
    key: 'resetScan',
    ...
    value: function startScan() {
      ...
      return new Promise(function (resolve, reject) {
        evothings.easyble.startScan(function (device) {
          if (_this.isMicrobitDevice(device)) {
            evothings.easyble.stopScan();
            resolve(device);
          }
        }, function (errorCode) {
    ...
    key: 'isMicrobitDevice',
    value: function isMicrobitDevice(device) {
      return device && device.name && (device.name.indexOf('MicroBit') > -1 || device.name.indexOf('micro:bit') > -1);
    }
  }, {
    key: 'connectToDevice',
    value: function connectToDevice(device) {
      return new Promise(function (resolve, reject) {
    ...
    key: 'readServices',
    value: function readServices(device) {
      var _this2 = this;

      return new Promise(function (resolve, reject) {
        device.readServices([_this2.config.ACCELEROMETER_SERVICE, _this2.config.DEVICE_INFO_SERVICE], resolve, function (errorCode) {
          reject(errorCode);
        });
      });
    }
    ...
    value: function startNotifications(device, callback) {
      var _this3 = this;
      ...
      return new Promise(function (resolve, reject) {
        ...
        _this3.writeNotificationDescriptor(device, _this3.config.ACCELEROMETER_DATA);

        // Set sensor period to 160 ms.
        var periodDataBuffer = new ArrayBuffer(2);
        new DataView(periodDataBuffer).setUint16(0, 500, true);
        device.enableNotification(_this3.config.ACCELEROMETER_DATA, callback, function (errorCode) {});
        resolve(device);
      });
    }
  }, {
    key: 'writeNotificationDescriptor',
    value: function writeNotificationDescriptor(device, characteristicUUID) {
      device.writeDescriptor(characteristicUUID, this.config.BLE_NOTIFICATION_UUID, new Uint8Array([1, 0]), function () {}, function (errorCode) {
  ...
  createClass(App, [{
    ...
    value: function initialize() {
      ...
      document.addEventListener('deviceready', function () {
        evothings.scriptsLoaded(_this.onDeviceReady.bind(_this));
      }, false);
    }
  }, {
    key: 'onDeviceReady',
    ...
    key: 'createNewStatus',
    value: function createNewStatus(status, timestamp, userId) {
    ...
    value: function startConnectTimer() {
      ...
      this.connectTimer = setTimeout(function () {
        _this3.showInfo('Scanning...\nPlease start the Microbit.');
      }, this.CONNECT_TIMEOUT);
      return this;
    }
  }, {
    key: 'stopConnectTimer',
    value: function stopConnectTimer() {
    ...
    value: function startScan() {
      ...
      this.microbit.startScan().then(function (device) {
        _this4.showInfo('Device found: ' + device.name + '. Connecting...');
        return _this4.microbit.connectToDevice(device);
      }).then(function (device) {
        _this4.showInfo('Connected - reading Microbit services...');
        _this4.stopConnectTimer();
        _this4.resetTotemPosition();
        return _this4.microbit.readServices(device);
      }).then(function (device) {
        _this4.showInfo('Starting notifications...');
        return _this4.microbit.startNotifications(device, _this4.handleAccelerometerValues.bind(_this4));
      }).then(function () {
        _this4.showInfo('Setup complete. Receiving data...');
      }).catch(function (err) {
        _this4.showInfo('Error: ' + err + '.');
      });
      return this;
    }
  }, {
    key: 'resetTotemPosition',
    value: function resetTotemPosition() {
      this.totemPosition = 1;
      this.oldTotemPosition = 1;
    }
  }, {
    ...
    value: function handleAccelerometerValues(data) {
    ...
    key: 'parseAccelerometerValues',
    value: function parseAccelerometerValues(data) {
